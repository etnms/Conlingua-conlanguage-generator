...
const createWord = (
  ...
  let rdmLength = Math.floor(Math.random() * (maxLetters - minLetters + 1)) + minLetters;
  let numberSyllables = Math.floor(Math.random() * maxNumberSyllables + 1);
  let randomWord = "";
  let previousLetter = "";
  ...
  for (let i = 0; i < numberSyllables; i++) {
    ...
    for (let i = 0; i < 3; i++) { // hard code 3 for now, but needs to identify different syllables structures
      ...
      if (i % 2 === 0) {
        ...
        if (consonantClusterSize > 1) {
          if (i % 2 === 0) i++;
          if (!consOnly) {
            if (i > rdmLength + consonantAdjust && i < minLetters) randomWord += vowels[rdmVowels];
          }
        }
      }
    }
